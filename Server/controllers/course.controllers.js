...
/**
 * @ADD_LECTURE
 * Adds a lecture to a course and uploads video or PDF to Cloudinary.
 */
export const addLectureToCourseById = asyncHandler(async (req, res, next) => {
    ...
    const lectureData = {
        ...
        description,
        lecture: {},
        contentType: 'document' // default to document
    }

    if (req.file) {
        try {
            // Determine if it's a video or document based on file extension
            const fileExtension = req.file.originalname.split('.').pop().toLowerCase();
            const isVideo = ['mp4', 'mov', 'avi', 'mkv', 'webm'].includes(fileExtension);
            const isPDF = ['pdf', 'doc', 'docx', 'ppt', 'pptx', 'txt'].includes(fileExtension);
            ...
            if (isVideo) {
                ...
                lectureData.contentType = 'video';
            } else {
                uploadOptions.resource_type = 'raw'; // for PDFs and documents
                lectureData.contentType = 'document';
            }
